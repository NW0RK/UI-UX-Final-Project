...
- Library animated heroes are stored separately in `animatedBannerUrl` and are displayed only when `settings.libraryAnimatedHeroesMode` allows them; static `bannerUrl` remains the fallback.
