...
- `StoreItemPage.jsx` resolves missing Steam App IDs by title, uses Steam reviews/details for media and rating display, prefers SGDB animated then static Store Item heroes before Steam hero/background fallbacks, falls back to IGDB screenshots only when no Steam match is available, and loads ITAD insights through `src/utils/itad.js`.
